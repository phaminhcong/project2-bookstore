        ...
        <div class="card-tools">
          <form action="{{ route('category.search') }}" method="GET">
            <div class="input-group input-group-sm" style="width: 200px;">
              <input type="text" name="search" class="form-control float-right" placeholder="Nhập tên khách hàng" value="{{ $searchTerm ?? '' }}">
      ...
      <div class="card-body table-responsive p-0">
        <table class="table table-hover text-nowrap">
          ...
          <tbody>
            ...
            <tr>
              <td>{{$row -> id}}</td>
              <td>{!! str_replace($searchTerm, '<span class="highlight">' . $searchTerm . '</span>', $row->email) !!}</td>
              <td>
